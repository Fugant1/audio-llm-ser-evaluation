...
class QwenOmni(BaseModel):
    """
    End-to-End Large Audio-Language Model (Qwen2-Audio / Qwen-Omni)
    using vLLM's native multimodal audio engine.
    """
    ...
    def load(self):
        try:
            ...
            raise ImportError(
                "vLLM is required to run QwenOmni with the vLLM backend. "
                "Install it via 'pip install vllm'."
            ) from e

        model_id = self.model_config.get("pretrained", "Qwen/Qwen2-Audio-7B-Instruct")
    ...
    def _format_prompt(self, prompt: str) -> str:
        """Injects audio placeholder tokens into prompt matching Qwen2-Audio chat syntax."""
        if "<|AUDIO|>" in prompt:
            return prompt
        return (
            "<|im_start|>system\nYou are a helpful speech emotion recognition assistant.<|im_end|>\n"
